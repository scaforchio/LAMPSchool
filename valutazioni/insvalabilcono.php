    ...
    if (!alunno_certificato_pei($id['idalunno'], $materia, $con))
    {
        ...
        if ($numvoti != 0)
        {
            if ($presentevoto & !$esistenti)
            {
                ...
                eseguiQuery($con,"delete from tbl_valutazioniabilcono where idvalint=$idvalint");
            } else
            {
        ...
        } else
        {
            if (!($presentevoto & !$esistenti))
            {
                eseguiQuery($con,"update tbl_valutazioniintermedie set voto=99 where giudizio<>'' and idvalint=$idvalint");
                eseguiQuery($con,"delete from tbl_valutazioniintermedie where idvalint=$idvalint and voto<>99");
                if ($esistenti)
                    echo "
					  <p align='center'>
