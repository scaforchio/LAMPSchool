...
$titolo = "Inserimento sub-obiettivi di comportamento";
$script = "";
...
$query = "delete from tbl_compsubob where idobiettivo=$idobiettivo";
$ris = mysqli_query($con, inspref($query)) or die ("Errore nella query: " . mysqli_error($con));


for ($no = 1; $no <= $maxabil; $no++)
{
    $subobiettivo = stringa_html("subob$no");

    $sintsubobiettivo = stringa_html("sintsubob$no");
    if ($sintsubobiettivo != "")
    {


        $query = "insert into tbl_compsubob(idobiettivo, numeroordine, sintsubob, subob)
                values('$idobiettivo','$no','$sintsubobiettivo', '$subobiettivo')";
...
print "<center><b>Inserimento sub-obiettivi effettuato!</b></center>";
