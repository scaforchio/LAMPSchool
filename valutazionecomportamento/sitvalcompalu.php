...
print ("
   <form method='post' action='sitvalcompalu.php' name='valabil'>
...
if ($alunno != "")
{
    ...
    $query = "select distinct tbl_valutazionicomp.data
	              from tbl_valutazionicomp where
	              tbl_valutazionicomp.idclasse=$idclasse
  ...
  //  print "ttt".count($datelez);
    ...
    $query = "select tbl_valutazioniobcomp.idsubob, tbl_valutazionicomp.idalunno,tbl_valutazionicomp.voto as votom,tbl_valutazioniobcomp.voto as votoob,tbl_valutazionicomp.data
	              ...
	              tbl_valutazioniobcomp.idvalcomp = tbl_valutazionicomp.idvalcomp and
	              tbl_valutazionicomp.idalunno=tbl_alunni.idalunno
	              and tbl_alunni.idclasse = tbl_classi.idclasse
	              and tbl_valutazionicomp.idclasse=$idclasse
	              and tbl_valutazionicomp.idmateria=$idmateria
   ...
   // print inspref($query);
    ...
    while ($nom = mysqli_fetch_array($ris))
    {
        ...
        $idobiettivo = $nom['idobiettivo'];
        $sintob = $nom['sintob'];
        $numeroordine = $nom['numeroordine'];

        $listavoti[] = $numeroordine . " " . $sintob;   // Aggiungo la definizione della comptenza al file

        print "<tr bgcolor='silver'>";
        print "<td>$numeroordine. $sintob</td>";
        for ($i = 0; $i <= count($datelez); $i++)
        {
            echo "<td align='center'>";
            echo "&nbsp;";
        ...
        while ($nomab = mysqli_fetch_array($risab))
        {
            ...
            $sintsubob = $nomab['sintsubob'];
            $numeroordinesub = $nomab['numeroordine'];
            $numvoti = 0;
            $totvoti = 0;
            print "<tr>";
            print "<td>$numeroordine.$numeroordinesub $sintsubob</td>";

            $listavoti[] = $numeroordine . $numeroordinesub . " " . $sintsubob;   // Aggiungo la definizione dell'obiettivo al file
            for ($i = 0; $i < count($datelez); $i++)
            {
                echo "<td align='center'>";
