...
stampa_pass_moodle_alunno($arr_idalu,$arr_utalu,$arr_pwalu);


function stampa_pass_moodle_alunno($arridalu,$arrutalu,$arrpwalu)
{
   ...
   foreach ($arridalu as $idalu)
   {
      ...
      $int=estrai_testo("passmoodlealu00",$con);
    ...
    //  $posY+=30;
      ...
      $schede->SetXY(10,$posY);
      $schede->SetFont('Times','',10);
      $comunicazione=estrai_testo("passmoodlealu02",$con);
      $schede->write(6,converti_utf8($comunicazione));
      $posY=$schede->GetY();
      $posY+=10;
      $schede->SetXY(10,$posY);
      ...
      if ((estrai_testo("ricevutapassalu",$con)=='yes') && ($posY<230))
      {
			$posY=230;
			$schede->Line(10,$posY,200,$posY);
			$schede->SetXY(50,$posY);
			$schede->SetFont('Times','',8);
			$schede->Cell(95,8,converti_utf8(estrai_testo("passalu03",$con)),NULL,1,"C");
			$posY+=10;
			$schede->SetXY(10,$posY);
			$schede->SetFont('Times','',10);
			$schede->MultiCell(190,8,converti_utf8(estrai_testo("passalu04",$con)." $alunno ".estrai_testo("passalu05",$con)));
			$posY+=15;
			$schede->SetXY(10,$posY);
			$schede->SetFont('Times','',10);
			$schede->Cell(90,8,converti_utf8("In fede."),NULL,1,"C");
			$posY+=10;
			$schede->SetXY(70,$posY);
			$schede->SetFont('Times','',10);
			$schede->Cell(100,8,converti_utf8("Firma genitore: ___________________________________"),NULL,1,"C");
			$posY+=10;
      }      
      
      $cont++;   
   }
   $nomefile="PWDMoodleAlunni.pdf";
