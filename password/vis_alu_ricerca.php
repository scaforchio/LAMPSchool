...
if (strlen($strcogn)>1 | strlen($strnome)>1)
{  
   ...
   { 
	 while($dati=mysqli_fetch_array($result))
      ...
      print("<td>".$dati['note']."</td>");
	   print("<td><a href='vis_alu_mod.php?idal=".$dati['idalunno']."&strcogn=$strcogn&strnome=$strnome'> Modifica </a></td>"); 
	   if (poss_canc_alu($dati['idalunno'], $con))
	      print ("<td><a href='alu_conf.php?idal=".$dati['idalunno']."&idcla=".$dati['idclasse']."'> Elimina </a></td>");
      else
         print ("<td>&nbsp;</td>"); 
      print("<td><a href='password/rigenera_passw_ins_sta.php?idalu=".$dati['idalunno']."'> Rigenera password</a></td>");    
       print("</tr>");
     }
